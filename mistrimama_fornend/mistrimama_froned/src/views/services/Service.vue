    ...
    <v-container fluid style="" v-if="categorys">
        ...
        <div class="date-area" v-if="viewarea == 'schedule'">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn color="secondary" @click="prevView" v-if="$route.params.offer != 'offer'"> PREV</v-btn>
        ...
        <div class="confirmation-area" v-if="viewarea == 'confirmation'">
            <v-card>
                ...
                <v-card-actions v-if="site_configs.refer ==1">
    ...
    export default {
        ...
        methods: { 
                ...
                for (var i = 0; i < this.offers.length; i++) {   // Offer for loop
                    if(this.offers[i].offers_type == 'discount_offer'){  
                        ...
                        for (var j = 0; j < this.categorys.length; j++) {  // Categores by  
                            ...
                            let promocode = this.offers[i].discount_offer.promocode;
                            let service_bit_id = this.offers[i].discount_offer.service_bit_id;
                            let serviceId = this.offers[i].discount_offer.service_id;
                                ...
                                for (var k = 0; k < this.services.length; k++) { 
                                    if(serviceId == this.services[k].id){  
                                        this.loadServiceBit(this.services[k].id); // service_id 
                                    } 
                                    for (var l = 0; l < this.services[k].serviceBits.length; l++) { 
                                        if(service_bit_id == this.services[k].serviceBits[l].id){  
                                            this.checkSelectedBit(this.services[k].serviceBits[l].id); // service_bit_id
                                            //console.log('199',this.services[k].serviceBits[l].id);
                                            this.selectedServiceBit = [this.services[k].serviceBits[l]]; 
                                            //this.totalPrice([this.selectedServiceBit]); 
                                            ...
                                            this.serviceBitsDialog = false; 
                                            this.viewarea = "schedule";
                                            this.onApply(promocode); 
                                            this.applyPromoCode();
                                        }                                        
                                    } 
                                } 
            ...
            loadServiceBit: function(serviceId) {
                //console.log('load_serviceId',serviceId);
                this.serviceBits = "";
                var arr = this.services; 
                var selectedService = this.services.find(arr => arr.id == serviceId)
                //console.log('load_selectedService',selectedService);
                this.serviceBits = selectedService.serviceBits;
                //console.log('load_serviceBits',this.serviceBits);
                this.selectedServices.push(selectedService);
                //console.log('load_selectedServices',this.selectedServices);
            ...
            checkSelectedBit: function(serviceBitId) {
                // alert(serviceBitId)
                //console.log('serviceBitId_s',serviceBitId);
                var arr = this.selectedServiceBit;
                //console.log('selectedServiceBit',arr);
            ...
            totalPrice(servicesBits) {
                //console.log('servicesBits',servicesBits);
