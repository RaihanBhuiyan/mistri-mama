        ...
        <v-container style="padding-top:30px; padding-bottom:30px; max-width: 900px;">
            <v-stepper alt-labels style="box-shadow:none" :value="viewarea == 'service' ? 1 : viewarea == 'schedule' ? 2 : viewarea == 'confirmation' ? 3 : ''">
                <v-stepper-header class="pt-4 pb-2">
                    <v-stepper-step step="1" :complete="viewarea == 'schedule' || viewarea == 'confirmation'">Service Selection</v-stepper-step>
                    <v-divider></v-divider>
                    <v-stepper-step step="2" :complete="viewarea == 'schedule' || viewarea == 'confirmation'">Schedule Time</v-stepper-step>
                    <v-divider></v-divider>
                    <v-stepper-step step="3" :complete="viewarea == 'as'">Confirmation</v-stepper-step>
                </v-stepper-header>
            </v-stepper>
                    ...
                    <v-flex v-for="category in categorys" :key="category.id" xs6 sm6 md2>
                        <v-card class="" :style="($route.params.category == category.slug) ? 'background-color: #333' : ''" elevation-1>
                            <div @click="ifChangeServiceCategory(category.slug)" style="margin: 2px; padding:15px; display:block; text-decoration: none; cursor: pointer;" class="text-center">
                                <span :style="($route.params.category == category.slug) ? 'color: #fff' : 'color: #333'">{{category.name}}</span>
                            </div>
                        </v-card>
                    </v-flex>
                </v-layout>
                <v-layout wrap v-if="services.length != 0" style="height:250px; overflow-y:scroll">
                            ...
                            <v-list-tile-content style="justify-content: space-around;">
                                ...
                                <ul>
                                    <li v-for="bit in getServiceBit(service.id)" :key="bit.id">
                                        <p class="caption white--text" style="margin:0;">{{bit.name}} : {{bit.qty}} {{bit.unit_type}}</p>
                                    </li>
                                </ul>
                            </v-list-tile-content>
                            <v-list-tile-action>
                                <v-list-tile-action-text class="caption" v-if="(totalPrice(getServiceBit(service.id)) > 0)">Price</v-list-tile-action-text>
                                <v-list-tile-action-text class="caption" v-if="(totalPrice(getServiceBit(service.id)) == 0)">Starting Price</v-list-tile-action-text>

                                <v-list-tile-action-text class="caption" v-if="(totalPrice(getServiceBit(service.id)) > 0)">৳ {{ totalPrice(getServiceBit(service.id)) }}</v-list-tile-action-text>
                                ...
                                <v-list-tile-action-text class="caption" v-if="(totalPrice(getServiceBit(service.id)) == 0)">
                                    <span v-if="((parseInt(service.minimum_mrp_price) - parseInt(service.minimum_price)) > 0)" class="minimum_mrp_price">
                                        ...
                                        <span class="discount_percentage">{{service.discount_percentage}}% off</span>
                                    </span>
                                    <span v-else class="minimum_price">৳ {{ service.minimum_price }} </span>
                    ...
                    <v-dialog v-model="serviceBitsDialog" scrollable max-width="900px" v-show="serviceBitsDialog" style="z-index:9999">
                        <v-card>
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn color="secondary" @click.stop="serviceBitsDialog = !serviceBitsDialog">Cancel</v-btn>
                                <v-btn color="primary" @click.stop="serviceBitsDialog = !serviceBitsDialog">Ok</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-dialog>
                </v-layout>
                            ...
                            <v-card-actions>
                                <v-list-tile-content>
                                    <v-list-tile-title class="font-weight-bold" style="font-size: 14px;">
                                        <span class="">Total Price: <span style="color: #ff8a00">৳ {{ ((totalPrice(selectedServiceBit) + extraChargeTotal()) - discount) }}&nbsp;</span></span>
                                        <span class="">&nbsp;Total Service Qty: <span style="color: #ff8a00">{{ totalQty(selectedServiceBit) }}</span></span>
                                     </v-list-tile-title>
                                </v-list-tile-content>
                            </v-card-actions>
                            <v-card-actions>
                                ...
                                <v-btn color="primary" v-if="selectedServiceBit.length != 0"  @click="nextView">Next</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-flex>
                </v-layout>
                ...
                <v-layout wrap style="margin-top:7px;">
                    <v-flex v-if="selectedServiceBit.length != 0">
                        <v-card>
                            <v-card-actions>
                                <v-list-tile-content>
                                    <v-list-tile-title class="font-weight-bold subheading" style="font-size:20">Total Price: <span style="color: #ff8a00">৳ {{ ((totalPrice(selectedServiceBit) + extraChargeTotal()) - discount) }}</span> Total Service Qty: <span style="color: #ff8a00">{{ totalQty(selectedServiceBit) }}</span></v-list-tile-title>
                                </v-list-tile-content>
                                <v-layout align-center justify-end>
                                    <v-btn color="secondary" @click="prevView">Back</v-btn>
                                    <v-btn color="primary" v-if="order.date && order.time" @click="nextView">Next</v-btn>
                                </v-layout>
                            </v-card-actions>
                        </v-card>
                    </v-flex>
                    ...
                    <v-flex xs12 sm12 md6>
                        <v-card class="elevation-0">
                            ...
                            <v-card-text v-if="((userType == 'login') || (userType == 'signup')) && (discount == 0)">
                                <v-card-text v-if="isLoggedIn == true">Use promo code here</v-card-text>
                                <v-card-text v-if="isLoggedIn == false">Please sign in for using promo code</v-card-text>
                                <v-btn class="mt-3" :disabled="isLoggedIn ? false : true" block color="primary" @click.stop="openPromoCodeDialog">Use promo code</v-btn>
                            </v-card-text>
                            <v-card-text v-if="userType == 'guest' && (discount == 0)">
                                <v-card-text>Use promo code here</v-card-text>
                                <v-btn class="mt-3" :disabled="(guestFormData.phone == '') ? true : false" block color="primary" @click.stop="openPromoCodeDialog">Use promo code</v-btn>
                            </v-card-text>
                        </v-card>
                    </v-flex>
                </v-layout>
                    ...
                    <v-flex v-if="selectedServiceBit.length != 0">
                        <v-card>
                            <v-card-actions>
                                ...
                                <v-layout align-center justify-end>
                                    <v-btn color="secondary" @click="prevView">Back</v-btn>
                                    <v-btn color="primary" @click="nextView">Confirm</v-btn>
                                </v-layout>
                            </v-card-actions>
                        </v-card>
                    </v-flex>
