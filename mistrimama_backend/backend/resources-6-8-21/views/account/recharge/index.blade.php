...
<div class="page-header pl-20 pr-20 pb-20 pt-0">
    ...
    <h1 class="page-title">Recharge Request</h1>

    <div class="page-header-actions">
        @if(count($rechargeRequests) > 0)
        <a class="btn btn-sm btn-primary btn-round waves-effect waves-classic" href="{{ route('recharge.export') }}">
            <i class="icon md-plus" aria-hidden="true"></i>
            <span class="hidden-sm-down">Download</span>
        </a>
        @endif
        <a class="btn btn-sm btn-primary btn-round waves-effect waves-classic" data-toggle="collapse" href="#importRechargeRequest"
            role="button" aria-expanded="false" aria-controls="importRechargeRequest">
            <i class="icon md-plus" aria-hidden="true"></i>
...
<div class="collapse" id="importRechargeRequest">
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title">Upload Recharge Request file</h3>
        </div>
        <div class="panel-body">
            <form action="{{route('recharge.import')}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="form-group mb-0">
                    <div class="input-group">
                        <span class="input-group-addon">Upload file here</span>
                        <input type="file" name="recharge_request_file" class="form-control" placeholder="" accept=".xlsx, .xls, .csv">
...
<div class="panel">
    <div class="panel-heading">
        <h3 class="panel-title">Approve Recharge Request</h3>
    </div>
    <div class="panel-body">
        @if(count($rechargeRequests) > 0)
        <div class="row">
            ...
            <table class="table">
                <thead>
                    <tr>
                        <th>SL</th>
                        <th>Action</th>
                        <th>Recharge Request Date & Time</th>
                ...
                <tbody>
                    @foreach ($rechargeRequests as $key => $request)
                    <tr>
                        <td>{{($key+1)}}</td>
                        <td>
                            @if ($request->status == 0)
                                <a onClick="return confirm('Sure to approve recharge request?')" href="{{route('recharge.approve', $request->id)}}" class="btn btn-xs btn-success">Approve</a>
                                <a onClick="return confirm('Sure to deny recharge request?')" href="{{route('recharge.deny', $request->id)}}" class="btn btn-xs btn-danger">Deny</a>
                            @else
                                Approved
                            @endif
                        </td>
        ...
        @else
        <p class="text-center panel-title">There is no recharge request found</p>
        @endif
    </div>
</div>
