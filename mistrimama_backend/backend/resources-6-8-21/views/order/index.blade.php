            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{($key+1)}}</td>
                        <td>
                            <a href="{{ route('order.edit', $order->id) }}" class="badge badge-primary btn btn-xs">Edit</a>
                            @if($order->status == 0)
                            <a href="{{route('order_accept', [$order->id, 'accept'])}}" class="badge badge-success btn btn-xs" onClick="return confirm('Are you sure you want to Approve?')">Accept</a>
                            <a href="{{route('allocate.sp', [$order->category_id, $order->id])}}" class="badge badge-primary btn btn-xs">Accept & Allocate</a>
                            @endif

                            @if($order->status == 1)
                            <a href="{{route('allocate.sp', [$order->category_id, $order->id])}}" class="badge badge-primary btn btn-xs">Allocate</a>
                            @endif

                            <a href="{{route('order_accept', [$order->id, 'fraud'])}}" class="badge badge-warning btn btn-xs" onClick="return confirm('Are you sure you want to Mark as Fraud?')">Mark as Fraud</a>
                            <a href="javaScript:;" data-toggle="modal" data-target="#orderCancelCommentModal{{ $order->id }}" class="badge badge-danger btn btn-xs">Cancel</a>
                        ...
                        <td>
                            <a href="{{route('order.show',$order->id)}}">Details</a>
                        </td>
...
@if(!empty($orders))
@foreach ($orders as $key => $order)
<div id="orderCancelCommentModal{{ $order->id }}" class="modal fade text-center" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form id="orderCancelCommentForm{{ $order->id }}" method="POST" action="{{ route('order_cancel', $order->id) }}">
                @csrf
                <div class="modal-body">
                <div class="form-group">
                    <label for="order_cancel_note{{ $order->id }}">Order Cancel Note</label>
                    <textarea class="form-control" id="order_cancel_note{{ $order->id }}" name="order_cancel_note" rows="3" required></textarea>
                </div>
                </div>
