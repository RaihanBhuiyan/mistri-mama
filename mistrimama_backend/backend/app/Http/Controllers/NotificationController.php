...
class NotificationController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            'notifications_title' => 'required|max:55',
            'notifications_for' => 'required|max:55',
            'description' => 'string'
        ]);
        $data['notification_image'] = NULL;
        $data['expire_date'] = NULL;
        $data['launch_url'] = $request->launch_url;
        onSignalFoUser($data);
        $local_path  = $_SERVER['DOCUMENT_ROOT'];

        if (!file_exists($local_path.'/'.env('APP_PATH')."/public/upload/notifications")) {
            mkdir($local_path.'/'.env('APP_PATH')."/public/upload/notifications", 0777, true);
        }
        if ($request->has('notification_image') && $request->notification_image != '') {
            $data['notification_image'] = base64_to_image($request->notification_image, 'upload/notifications');
        } 
